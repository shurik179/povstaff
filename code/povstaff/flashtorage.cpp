...
int readLine(File &f, char  l[]){
    ...
    while (f.available()&&(pos<MAX_LINE_LENGTH)) {
        ...
        //otherwise, add the character to string, but skip whitespace, and do not wrtie more than MAX_LENGTH
        // note: accordign to docs, \r shoudl have been caught by isWHiteSpace - but it looks like in fact it doesn't, 
        // so we have to check for it separately
        if ((!isspace(c))&&(pos<MAX_LINE_LENGTH)) { l[pos++]=c;}
    }
